...
## 現在のステータス (2026-07-09 更新)

**2026-07-09 セッションの成果(コンテンツ拡充・デプロイ済み):**

- **★ep-12〜16 を一気に5本追加（国際を厚く）**: 申し送り「次は国際優先」に従い5本量産。**ep-12 比較優位(国際)**「なぜ何でも作れる国が輸入するのか」/**ep-13 コモンズの悲劇(国際)**「共有の漁場の魚が獲り尽くされる」/**ep-14 選好の偽装(社会)**「盤石な体制・大流行が突然崩れる」/**ep-15 経路依存性(テク)**「使いにくいQWERTYがなぜ残るのか」/**ep-16 生存者バイアス(科学)**「帰還爆撃機の弾痕に装甲を足すな」。各回フル成果物(summary＋ラジオ台本45〜51行＋**各回固有SVG図解**＋読みマニフェスト＋音声)。**全話 validate通過＋読みゲート `--gate` が MISMATCH0/exit0＋音声生成→Release アップロード済み**。読みゲートで拾って直した誤読(=ゲートが実際に効いた実績): 特化→トクカ・持ち主→モチシュ(文中結合で priority10)・後知恵→アト/チエ・実際→ジツギワ を **`data/voicevox_dict.json` に恒久登録**、地頭/一声/QWERTY綴り字/依存(いぞん)/消えた(濁り化け)は**台本言い換え**で回避。**プールは 16/50本、残り34本**。ジャンル分布: 経済2・テクノロジー3・**国際3**・社会3・科学3・文化2(バランス改善)。図解: 比較優位=特化で伸びる総量の山/コモンズ=みんなの海から引き出す矢印と縮む資源/選好の偽装=水面上は全員賛成・水面下は割れた本音/経路依存=二つの谷の地形(高い谷にロックイン)/生存者バイアス=帰還爆撃機の弾痕図。**`js/diagrams.js` 変更につき sw.js v10**。未配信モデル83(十分)
- **ep-11「なぜ人は"見せること"にお金を払うのか」(シグナリング/文化)を新規追加(commit `1052752`)**: 台本量産フェーズの本命。題材=学歴・ブランド品・クジャクの羽=「偽物には割に合わないコストの高い証拠」が質を示す(シグナリング/ハンディキャップ原理)。**新スキーマの1話フル成果物**を揃えた: (a) `content/ep-11.json` の summary(選択式hook3択/定義/メカニズム4/**他分野応用3件=生物・広告・人間関係**/使いどころ/限界/適用クイズ)＋ラジオ台本50行(「今日のレンズ」呼称・フェイ×ヒナタの行列ネタで入る) (b) `js/diagrams.js` に **ep-11 固有SVG図解**(「コストの壁」を高い質は軽々越え、低い質は割に合わず引き返す=分離のふるい。箱の羅列でない) (c) 音声(VOICEVOX、410.87秒/2.46MB、Release `audio-v1` アップロード済み)。**validate通過→読み検査(手順6)で誤読1件=「地頭」がジトウ(歴史用語)に化けたのを台本言い換え(「もともと得意な人」)で解消・再検査クリーン→make_audio→アップロード→commit/push→Actionsデプロイ成功→ライブ反映をWebFetch確認(content配信・音声はaudio/ep-11.mp3をHTTP200/audio/mp3/2.46MBで同一オリジン配信=iOS再生可)**。`js/diagrams.js`(アプリ本体JS)を変更したので**sw.js v9**に更新。**プールは 11/50本、残り39本**。ジャンル分布更新: 社会×2・経済×2・テクノロジー×2・科学×2・国際×1・**文化×2**(申し送り「国際・文化を厚めに」のうち文化を1本前進。**次は国際を厚めに**)。未配信モデルは十分残(signalingにdeliveredOn=2026-07-09記入)
- **読み誤りチェックを「目視ダンプ」から「読みゲート」に刷新＋共有化(同2026-07-09)**: 「世界の名著」で先行実装した読みゲート(誤読申告マニフェスト＋合成前の機械チェック)を kaizodo にも移植し、**純ロジックを `_shared/reading-gate/` に集約して両プロジェクト共用**にした。構成: (a)共有コア3モジュール `reading_core.py`(読み書式＋fingerprint)/`reading_gate.py`(find_mismatches=申告カナが実読みに無ければMISMATCH/find_suspects=既知トラップ・列挙単漢字の申告漏れSUSPECT)/`readings_manifest.py`(申告ファイル読込検証)＋純ロジックの単体テスト19件。各プロジェクトの scripts は先頭で `sys.path.insert(_shared/reading-gate)` するだけ(`parents[2]`)。(b)各回に **`content/ep-N.readings.json`(誤読申告=surface/kana配列。kanaはVOICEVOX綴り＝長音オ/ウのまま)を必須成果物化**。`scripts/check_readings.py --gate ep-N` が MISMATCHで exit 1。トラップ表 `data/reading_traps.json`。(c)`validate.mjs` がマニフェスト存在を検査、GENERATION.md 手順6をゲート方式に全面改訂、kaizodo用ゲートドライバ試験 `scripts/tests/test_gate_driver.py` 追加。**検証: 世界の名著の既存30テスト緑維持(抽出で無破壊)＋共有19＋kaizodo4テスト緑、validate全11話OK、実VOICEVOXで `--gate ep-11` が MISMATCH0/exit0**。ep-1〜10は薄いプレースホルダ `[]`、ep-11は申告5語(身銭/消耗/一見/十分/生物)。**注意: `_shared/`・世界の名著・ワークスペースルートはこのPCではgit非管理**なので、これらの変更はディスク上のみ(kaizodoのみコミット)。世界の名著側の同期コミットはユーザー側の管理に委ねる
- **残(このセッションからの申し送り)**: ①iPhone実機で音声再生を最終確認(ep-11含む。SW v9更新後) ②まとめページ実機UX確認 ③ep-11の固有SVG図解を実機で見映え確認(壁のふるいの絵。QA5項準拠で作図したが実レンダリング未目視) ④**次の題材は国際ジャンルを優先** ⑤新エピソードは必ず読みゲート(`--gate`)を緑にしてから音声化(GENERATION.md手順6)(未使用の好適モデル例: comparative-advantage比較優位・tragedy-of-commonsコモンズの悲劇・preference-falsification選好の偽装・demographic-dividend人口ボーナス)
...
- **図解を「各回固有のSVG」に刷新(同2026-07-08・最終形は commit `851dc1c`・デプロイ済み)**: 最初に構造化DSL6種へ刷新(commit `355b995`)したが、ユーザーから「箱を並べただけ＝グラフィカルでない」と却下。`世界の名著` の**図解ルール**(`docs/コンテンツ改善-2026-07-08.md` §2 で確定)に従い、**型枠を廃して概念ごとに"一番直感で入る絵"を固有SVGで設計**する方式へ全面転換。目的は「概念を直感で理解させる」こと。**サンプル2枚をユーザー承認(GO)後、残り8枚も作成**。実装: **`js/diagrams.js` 新規**(epId→固有SVG文字列のmap。共通marker `AH/AHr/AHm`)、`matome.js` は `diagrams[epId]` を `.figbox` に注入(旧 `diagramHTML`/DSL撤去、`richText` のみ残す)、`content/ep-N.json` は `summary.diagram` 撤去→`summary.diagramNote`(核心一言)、`css/style.css` は `.dgm-*` 撤去→`.figbox`+`.dgm-note`、`validate.mjs` は `js/diagrams.js` を読み各回SVGの存在を検証、`sw.js` v8。図解: 行列=雪だるま/ナッシュ=浜辺の売店/ネットワーク効果=つながりの網/創発=逆走する渋滞波/資源の呪い=断面図/確証バイアス=2×2/レモン市場=良品が抜けレモン残る箱/二次効果=波紋/モラルハザード=天秤/創造的破壊=交差するS字。配色は深夜FM(良い緑/危険朱/主役琥珀/明朝)。**全10話 validate 通過＋stub DOM 描画テスト全PASS、デプロイ→ライブ確認済み**。プレビュー(実SVG全10話)を Artifact 公開。**図解QAルール5項は GENERATION.md「図解の作り方」に明記。新規回は必ず固有SVGで(箱の羅列に戻さない)**
...
0. **(2026-07-08 追加) 実機確認**: ①iPhoneで今日の放送の音声が鳴るか(PWAを再読み込み=SW **v8**更新後) ②まとめタブの表示(固有SVG図解10枚・選択式仮説・操作バー固定・応用)。OKなら iOS対応と一連のUX刷新は完了
1. **★次セッションの本命=コンテンツ拡充(台本量産)**(GENERATION.mdに従う。**ep-1〜10 済み=10/50本、残り40本**。数セッションに分割、1本ごとに`node scripts/validate.mjs`必須)。**新スキーマの1話に必要な成果物(2026-07-08確定)**: (a) `content/ep-N.json` の `summary`(hook=event/question/**choices3択**・definition・mechanism・**applications(他分野応用)**・usage・limits・quiz・**diagramNote**)＋`radio`(台本。「今日のレンズ」と呼ぶ。「メンタルモデル」とは言わない) (b) **`js/diagrams.js` にその回固有のSVG図解**(箱の羅列禁止＝概念を直感で伝える絵。GENERATION.md「図解の作り方」＋QA5項に従う) (c) **`content/ep-N.readings.json`(誤読申告)＋読みゲート `--gate` 緑**(GENERATION.md手順6。誤読ゼロでも `[]` を置く) (d) 音声(手順2)。題材は「10年後も思考訓練として成立する」実在の出来事/現象。ジャンルの偏りに注意(既出: 社会×2・経済×2・テクノロジー×2・科学×2・国際×1・文化×1。次は**国際・文化**を厚めに)。重複回避は`content/index.json`のpool台帳と`data/models.json`の`deliveredOn`で管理
